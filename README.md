...
### ⚠️ CRITICAL: Copy Your Hardware Config

**BEFORE you can use this flake, you MUST copy your hardware configuration.** The file `hosts/ghost/hardware-configuration.nix` contains a placeholder and must be replaced with your actual configuration.
...
# 2. Copy your hardware configuration (REQUIRED)
cp /etc/nixos/hardware-configuration.nix hosts/ghost/

# 3. Verify the file was copied
...
Nix flakes use "pure evaluation mode" which cannot access files outside the flake directory (including symlinks to `/etc/nixos/`). By copying the file into the flake, it becomes part of the evaluation.
...
You forgot to copy the file. Run:
```bash
cd ~/.config/nixme
cp /etc/nixos/hardware-configuration.nix hosts/ghost/
```
